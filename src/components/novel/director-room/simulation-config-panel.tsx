...
import { WORD_BUDGET_PRESETS, type SimulationMode, MODE_VISUAL_INFO } from "@/lib/novel/director-room/types"
...
export function SimulationConfigPanel({ onStart }: SimulationConfigPanelProps) {
  ...
  const directorEnabled = useDirectorRoomSimulationStore((s) => s.directorEnabled)
  const setMode = useDirectorRoomSimulationStore((s) => s.setMode)
  const setUserIdea = useDirectorRoomSimulationStore((s) => s.setUserIdea)
  const setTargetWords = useDirectorRoomSimulationStore((s) => s.setTargetWords)
  const setSourceChapters = useDirectorRoomSimulationStore((s) => s.setSourceChapters)
  const setSimulationRounds = useDirectorRoomSimulationStore((s) => s.setSimulationRounds)
  const setDirectorEnabled = useDirectorRoomSimulationStore((s) => s.setDirectorEnabled)
