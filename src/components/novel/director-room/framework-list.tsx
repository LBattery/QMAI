...
import {
  useDirectorRoomSimulationStore,
} from "@/stores/director-room-simulation-store"
import { deleteFramework, loadFrameworks } from "@/lib/novel/director-room/framework-store"
...
export function FrameworkList({
  ...
  const frameworks = useDirectorRoomSimulationStore((s) => s.frameworks)
  const setFrameworks = useDirectorRoomSimulationStore((s) => s.setFrameworks)
  const binding = useDirectorRoomSimulationStore((s) => s.binding)
  const setBinding = useDirectorRoomSimulationStore((s) => s.setBinding)
  const currentFrameworkId = useDirectorRoomSimulationStore(
    (s) => s.currentFramework?.id ?? null,
  )
  const listRefreshKey = useDirectorRoomSimulationStore((s) => s.listRefreshKey)
