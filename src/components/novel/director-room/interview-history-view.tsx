...
import { loadInterviews, deleteInterview } from "@/lib/novel/director-room/interview-store"
import { exportInterview } from "@/lib/novel/director-room/interview-export"
import { deserializeSimulationSnapshot } from "@/lib/novel/director-room/simulation-serializer"
import { loadSimulationResults } from "@/lib/novel/director-room/framework-store"
import type { SavedInterview } from "@/lib/novel/director-room/interview-store"
import type { NovelAgent } from "@/lib/novel/director-room/types"

export function InterviewHistoryView() {
  const projectPath = useWikiStore((s) => s.project?.path)
  const showInterviewHistory = useDirectorRoomSimulationStore((s) => s.showInterviewHistory)
  const savedInterviews = useDirectorRoomSimulationStore((s) => s.savedInterviews)
  const viewingInterview = useDirectorRoomSimulationStore((s) => s.viewingInterview)
  const setShowInterviewHistory = useDirectorRoomSimulationStore((s) => s.setShowInterviewHistory)
  const setSavedInterviews = useDirectorRoomSimulationStore((s) => s.setSavedInterviews)
  const setViewingInterview = useDirectorRoomSimulationStore((s) => s.setViewingInterview)
  ...
  const setContinuingInterviewId = useDirectorRoomSimulationStore((s) => s.setContinuingInterviewId)
  const setActiveChatAgent = useDirectorRoomSimulationStore((s) => s.setActiveChatAgent)
  const setAgentChatMessages = useDirectorRoomSimulationStore((s) => s.setAgentChatMessages)
