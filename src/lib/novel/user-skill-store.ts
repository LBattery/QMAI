...
import { getFileName, getFileStem, joinPath } from "@/lib/path-utils"
import {
  normalizeUserSkill,
  type SkillCategory,
  type SkillKind,
...
export async function importLinkedSkill(
  ...
  if (isFile) {
    const fileName = getFileName(folderOrFilePath)
    ...
    const folderName = getFileName(folderOrFilePath)
    name = folderName
    try {
      const skillMdPath = joinPath(folderOrFilePath, "SKILL.md")
...
export async function loadUserSkillConfig(projectPath: string | null | undefined): Promise<UserSkillConfig> {
  ...
  const configPath = joinPath(projectPath, USER_SKILL_CONFIG_FILE)
  try {
