...
const THREE_POINT_ONE_FIVE_CHANGELOG: ChangelogEntry = {
  ...
  highlights: {
    ...
    zh: [
      ...
      "【写作设置生效修复】批量去 AI 味并发数接到当前章节队列；单章目标字数钳制为 1000–10000（与写作设置 UI 一致）；修改反馈窗口与对话历史立即落盘；检索 TOPK 同时约束图谱检索；前情分析在标准模式也可生效；删除已被工作流模式架空的「深度模式：AI 审稿与自动返修」开关（含配置字段）",
    ],
  },
};
